import logo1 from './Pictures/logo.svg';
import searchIcon1 from './Pictures/search.svg';
import profileIcon1 from './Pictures/profile.svg';
import bagIcon1 from './Pictures/bagSmall.svg';
import ham1 from './Pictures/ham.svg';
import exitLogo1 from './Pictures/exit.png';

export const logo = logo1;
export const searchIcon = searchIcon1;
export const profileIcon = profileIcon1;
export const bagIcon = bagIcon1;
export const ham = ham1;
export const exitLogo = exitLogo1;
